...
def dislike_article(requeat, slug):
    article = Article.objects.get(slug=slug)

    if requeat.user not in article.dislikes.all():
        article.dislikes.add(requeat.user)
        article.likes.remove(requeat.user)
    elif requeat.user in article.dislikes.all():
        article.dislikes.remove(requeat.user)
    return redirect('article_detail', slug=slug)
